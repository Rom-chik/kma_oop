#pragma once // developed by Roman Antoshchuk on 18.02.2024
#include <iostream>
#include <string>
using namespace std;
...
public:
    struct WrongDate {
        ...
        WrongDate(int d, int m, int y) : _day(d), _month(m), _year(y) {};
    };
    ...
    const string getMonth() const { return monthNames[_month - 1]; }
...
private:
    ...
    friend ostream& operator<<(ostream& os, const Date& d) {
            os << d._day << "-" << d.getMonth() << "-" << d._year;
            return os;
    }

    friend ostream& operator<<(ostream& os, const WrongDate& bd) {
            os << "{" << bd._day << ", " << bd._month << ", " << bd._year << "}"; //wrong date
            return os;
    }
...
inline const Date Date::operator++() {
